...
Latest version can do digital receipt to customer. Well, all the program does is send the receipt plus phone number to my home server. A script on the server converts the receipt to pdf, gives it a unique name and sends the customer an sms with a dowload link. Looks professional but is just homebrew. It all happens when you press <i>Link</i> in the receipt routine.
